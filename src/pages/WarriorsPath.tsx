    ...
    <div className="h-full flex flex-col bg-gradient-to-b from-dojo-dark via-dojo-navy to-dojo-dark overflow-auto">
      ...
      <div className="flex items-center justify-between px-4 pt-4 pb-2">
        ...
        >
          <WarriorAvatar
            ...
            animate={false}
          />
          <span className="font-baloo text-sm font-bold text-white">{activeProfile.name}</span>
        </motion.button>
      ...
      <div className="flex-1 px-4 pb-8 overflow-auto">
        <div className="max-w-md mx-auto">
          <motion.h2
            className="font-baloo text-xl font-extrabold text-amber-400 mb-4 text-center"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          >
            ⛩️ {dojo1.name} — {dojo1.cefr}
          </motion.h2>
          ...
          <div className="flex flex-col gap-3">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className="w-10 h-10 rounded-xl flex items-center justify-center font-baloo font-extrabold text-sm"
                      style={{
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-baloo font-bold text-white text-sm truncate">
                          {unit.title}
                        </span>
                        <BeltBadge belt={unit.belt} size="sm" earned={isAccessible} />
                      </div>
                      ...
                      {isAccessible && (
                        <div className="mt-2 flex items-center gap-2 flex-wrap">
                          <div className="flex gap-0.5 text-sm" aria-hidden>
                            {[0, 1, 2].map((i) => (
                              <span key={i} className={i < stars ? 'text-amber-400' : 'text-white/15'}>
                                {i < stars ? '★' : '☆'}
                              </span>
                            ))}
                          </div>
                          {bestAcc > 0 && (
                            <span className="font-pixel text-[0.65rem] text-white/50">
                              {Math.round(bestAcc * 100)}%
                            </span>
                          )}
                          {showNotTrained && (
                            <span className="font-poppins text-[0.65rem] text-amber-400/80">
                    ...
                    {isAccessible && (
                      <div className="flex flex-col items-end gap-0.5">
                        ...
                        {bestAcc > 0 && (
                          <span className="font-pixel text-[0.55rem] text-white/35">best {Math.round(bestAcc * 100)}%</span>
                        )}
                      </div>
                    )}
                  </div>
            ...
            {(() => {
              ...
              const allUnitsTrained = dojo1.units.every((u) => (bestByUnit.get(u.id) ?? 0) > 0);
              const reachedEnd = activeProfile.currentUnit >= lastUnitId;
              const beltTestReady = reachedEnd && allUnitsTrained;
              return (
                <motion.div
                  className="rounded-2xl border-2 border-dashed border-amber-400/30 p-4 text-center"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: reachedEnd ? 1 : 0.3 }}
                  transition={{ delay: 0.5 }}
                >
                  <span className="font-baloo text-amber-400 font-bold">🏯 Belt Test — Prueba de Cinturón</span>
                  {beltTestReady ? (
                    <DojoButton
                      size="sm"
                      className="mt-3"
